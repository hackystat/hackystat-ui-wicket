...
public class TelemetrySession implements Serializable {
  ...
  /**
   * The last parameter key that is required. The telemetry parameters key is optional because not
  ...
  public List<ParameterDefinition> getParameterList(String telemetryName) {
    ...
    if (telemetryName != null) {
      TelemetryChartDefinition teleDef = this.getTelemetryDefs().get(telemetryName);
      if (teleDef == null) {
        TelemetryClient client = ProjectBrowserSession.get().getTelemetryClient();
        try {
   ...
   */
  /*
   * public String getStartDateString() { SimpleDateFormat format = new
   * SimpleDateFormat(ProjectBrowserBasePage.DATA_FORMAT, Locale.ENGLISH); return format.format(new
   * Date(this.startDate)); }
   */
   ...
   */
  /*
   * public String getEndDateString() { SimpleDateFormat format = new
   * SimpleDateFormat(ProjectBrowserBasePage.DATA_FORMAT, Locale.ENGLISH); return format.format(new
   * Date(this.endDate)); }
   */
  ...
  public void updateDataModel() {
    ...
    dataModel.setModel(getStartDate(), getEndDate(), selectedProjects, telemetryName, granularity,
        parameters, ((ProjectBrowserApplication) ProjectBrowserApplication.get())
            .getTelemetryHost(), userEmail, ProjectBrowserSession.get().getPassword());
  ...
  public boolean loadPageParameters(PageParameters parameters) {
    ...
    if (parameters.containsKey(START_DATE_KEY)) {
      ...
      try {
        this.startDate = Tstamp.makeTimestamp(startDateString).toGregorianCalendar()
            .getTimeInMillis();
      }
      catch (Exception e) {
        isLoadSucceed = false;
    ...
    if (parameters.containsKey(END_DATE_KEY)) {
      ...
      try {
        this.endDate = Tstamp.makeTimestamp(endDateString).toGregorianCalendar().getTimeInMillis();
      }
      catch (Exception e) {
        isLoadSucceed = false;
